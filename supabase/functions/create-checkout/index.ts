...
serve(async (req) => {
  ...
  try {
    ...
    if (!planName || !billingCycle) {
      throw new Error("Missing planName or billingCycle");
    }
    ...
    const pricingConfig = {
      Basic: {
        ...
        annual: { amount: 9990, interval: "year" }
      },
      Pro: {
        monthly: { amount: 2999, interval: "month" },
        annual: { amount: 29990, interval: "year" }
      },
      Enterprise: {
        monthly: { amount: 9999, interval: "month" },
        annual: { amount: 99990, interval: "year" }
      }
    };

    const pricing = pricingConfig[planName as keyof typeof pricingConfig]?.[billingCycle as keyof typeof pricingConfig.Basic];
    if (!pricing) {
      ...
      line_items: [
        {
          price_data: {
            ...
            recurring: { interval: pricing.interval as any },
          },
          quantity: 1,
        },
      ],
